...
class PrefixHandler:
    ...
    def geohash_dict_to_df(self, geohash_short_prefix: Dict) -> pd.DataFrame:
        ...
        """
        geohash_lookup = [
            {"geohash": key, "unique_prefix": value}
            for key, value in geohash_short_prefix.items()
        ]
    ...
    def prefix_exists(self, geohash_list: List, geohash_prefix: str) -> bool:
        ...
        """
        try:
            return geohash_list[bisect_left(geohash_list, geohash_prefix)].startswith(
                geohash_prefix
            )
        except IndexError:
            return False
    ...
    def get_shortest_prefix(
        self, geohash_prefixes_map: List, geohash_list: List, geohash_code: str
    ) -> Dict:
        ...
        geohash_left = geohash_idx - 1 if geohash_idx != 0 else 0
        geohash_right = geohash_idx + 2
        geohash_aux_list = geohash_list[geohash_left:geohash_right]
    ...
    def geohash_mapper(self, geohash_list: List) -> pd.DataFrame:
        ...
        for geohash_code in geohash_list:
            ...
            geohash_shortest_map = self.get_shortest_prefix(
                geohash_prefixes_map, geohash_list, geohash_code
            )
